...
class BucketContainer extends StatelessWidget {
  ...
  BucketContainer({this.name, this.activity, this.budget, this.priority, this.docId, this.userId, this.bucketStatus, this.date, this.picture, this.message, this.userEmail});
          ...
          child: FlatButton(
            onPressed: (){
              Navigator.push(context, MaterialPageRoute(builder: (context)=>BucketInfoPage(name: name,
                activity: activity, priority: priority, budget: budget, docId: docId, userId: userId, date: date, picture: picture, message: message, userMail: userEmail,)));
            },
            child: Container(
